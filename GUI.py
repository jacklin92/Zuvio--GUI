...
class CheckInThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            scripts = soup.find_all(
                "script", string=re.compile('var accessToken = "(.*?)";')
            )
        ...
        if course_json["status"]:
            ...
            for course_data in course_json["courses"]:
                if "Zuvio" not in course_data["teacher_name"]:
                    self.check_in_signal.emit(
                        course_data["course_name"] + " - " + course_data["teacher_name"]
                    )
            already_checked = []
            while self.isLoop:
                ...
                for course_data in course_json["courses"]:
                    ...
                    if "Zuvio" not in course_data["teacher_name"]:
                        ...
                        if rollcall_id != "":
                            result = course_data["course_name"] + self.checkIn(
                                user_id, accessToken, rollcall_id
                            )
    ...
    def check(self, course_ID):
        ...
        soup = BeautifulSoup(response.content, "html.parser")
        scripts = soup.find_all(
            "script", string=re.compile("var rollcall_id = '(.*?)';")
        )
...
class MyWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Zuvio Auto Check-in By WJ&Kalvin")
    ...
    def start_check_in(self):
        ...
        if not account or not password or not lng or not lat:
            self.result_text.setPlainText("請輸入信箱、密碼、經度和緯度")
            return
